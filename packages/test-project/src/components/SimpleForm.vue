...
export default {
  ...
  setup () {
    ...
    let v$ = useVuelidate(
      {
        name: {
          required, minLength: minLength(4, 'Name'), asyncValidator
        },
        social: {
          github: { minLength: minLength(4, 'Github handle') },
          twitter: { asyncValidator, minLength: minLength(6, 'Twitter handle') }
        }
      },
      { name, social },
      { $autoDirty: true }
